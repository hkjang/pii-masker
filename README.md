...
비동기 작업(`POST /v1/jobs`)은 동시에 `PII_MASKER_MAX_CONCURRENT_JOBS`개(기본 4개)만 실행합니다. 초과분은 `queued` 상태로 대기하다가 순서대로 실행되며, 대기 중인 작업은 문서 바이트를 메모리에 들고 있지 않고 차례가 오면 저장된 입력 파일을 다시 읽습니다. 업로드를 한꺼번에 몰아넣어 서버 메모리를 고갈시키는 것을 막기 위한 제한입니다. 작업의 `created_at`은 업로드가 접수된 시각으로 고정되며 대기 후 실행이 시작돼도 바뀌지 않으므로, `updated_at`과의 차이로 대기 시간을 잴 수 있습니다.
